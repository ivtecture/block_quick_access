...
|---|---|---|---|
| `web` | `moodlehq/moodle-php-apache:8.2` | Apache + PHP 8.2 с ядром Moodle 4.5 (ветка `MOODLE_405_STABLE`, клонируется при первом запуске); код плагинов примонтирован из репозитория (`blocks/quick_access`, `blocks/course_progress`) | http://localhost:8080 |
...
В репозитории два блока:

| Блок | Папка | Назначение |
|---|---|---|
| **Quick access** («Быстрый доступ») | корень репозитория | Список полезных ссылок (Дашборд, Мои курсы, Календарь, Главная) + «Администрирование» для админов |
| **Course progress** («Прогресс по курсу») | `course_progress/` | Прогресс текущего пользователя по курсу: процент выполненных элементов курса (через «Отметку о выполнении элементов курса»). Работает только на страницах курса |

Оба примонтированы в контейнер `web` как `blocks/quick_access` и
`blocks/course_progress` соответственно.
