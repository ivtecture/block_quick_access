# Быстрые ссылки (block_quicklinks) — плагин блока для Moodle 4.5

Плагин блока **«Быстрые ссылки» (Quick links)** для Moodle 4.5: блок для страницы
курса, в котором **преподаватель** добавляет произвольный список ссылок
(название → URL) через форму настройки блока, а **студенты** видят этот список
в режиме только для чтения.

> Это отдельная ветка `quicklinks` репозитория [block_quick_access](https://github.com/ivtecture/block_quick_access).
> Ветка `main` содержит базовый плагин `block_quick_access`, эта ветка — плагин
> `block_quicklinks`, созданный по той же структуре и в том же dev-окружении.
...
Дашборд/курс → включить **режим редактирования** → «Добавить блок» → **«Quick links»**
(или **«Быстрые ссылки»**, если включён русский интерфейс). Затем нажать
шестерёнку на блоке → «Настроить блок "Быстрые ссылки"» и добавить ссылки
(название + URL), используя кнопку **«Добавить ещё одну ссылку»**. Студенты
видят список ссылок без возможности редактирования.
...
```
block_quicklinks/  (корень репозитория, ветка quicklinks)
